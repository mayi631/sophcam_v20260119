...
extern char g_button_labelflash[32];

static uint8_t flash_Current_Index_s = 0;

lv_obj_t *photomenu_flash_s;

void photo_setFlash_Index(uint8_t index)
{
    flash_Current_Index_s = index * 2;
}

uint8_t photo_getFlash_Index(void)
{
    return flash_Current_Index_s / 2;
}

void photo_setFlash_Label(const char* plabel)
{
    memset(g_button_labelflash, 0, sizeof(g_button_labelflash));
    strncpy(g_button_labelflash, plabel, sizeof(g_button_labelflash));
}

char* photo_getFlash_Label(void)
{
    return g_button_labelflash;
}
...
static void flash_win_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, flash_Del_Complete_anim_cb);

    lv_anim_start(&Delete_anim);
}

static void flash_btn_back_event_handler(lv_event_t *e)
{
...
void flash_SelectFocus_OK(lv_event_t *e, lv_obj_t *obj)
{
    ...
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(parent, flash_Current_Index_s);

    for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
        if(i == flash_Current_Index_s) {
...
static void flash_btn_event_handler(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    switch(code) {
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                ...
                if(lv_obj_get_child(parent, i) == btn_clicked) {
                    MESSAGE_S Msg = {0};
                    flash_Current_Index_s = i;
                    flash_SelectFocus_OK(e, NULL);
                    // 获取按钮标签文本
                    lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
                    if(label && lv_obj_check_type(label, &lv_label_class)) {
                        const char *txt = lv_label_get_text(label);
                        if(txt) strncpy(g_button_labelflash, txt, sizeof(g_button_labelflash));
                    }
                    Msg.topic = EVENT_MODEMNG_SETTING;
                    Msg.arg1  = PARAM_MENU_FLASH_LED;
                    Msg.arg2  = flash_Current_Index_s / 2;
...
static void flash_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("flash_click_callback\n");
...
static void flash_menu_callback(void)
{
    MLOG_DBG("flash_menu_callback\n");
    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
        &g_ui.screen_SettingSmileDectection_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0,
        0, false, true);
}
...
void photoMenu_Flash(lv_ui_t *ui)
{
    ...
    if(photomenu_flash_s != NULL) {
        if(lv_obj_is_valid(photomenu_flash_s)) {
            MLOG_DBG("photomenu_flash_s 仍然有效，删除旧对象\n");
            lv_obj_del(photomenu_flash_s);
        } else {
            MLOG_DBG("photomenu_flash_s 已被自动销毁，仅重置指针\n");
        }
        photomenu_flash_s = NULL;
    }
    // Write codes scr
    photomenu_flash_s = lv_obj_create(NULL);
    if(!photomenu_flash_s) return;

    lv_obj_set_size(photomenu_flash_s, 640, 480);
    lv_obj_set_scrollbar_mode(photomenu_flash_s, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_style_pad_all(photomenu_flash_s, 0, 0);
    lv_obj_add_event_cb(photomenu_flash_s, gesture_event_handler, LV_EVENT_GESTURE, ui);

    // Write style for scr, Part: LV_PART_MAIN, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(photomenu_flash_s, LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(lv_layer_bottom(), LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(photomenu_flash_s, LV_OPA_100, LV_PART_MAIN);
    lv_obj_set_style_bg_color(photomenu_flash_s, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes cont_top (顶部栏)
    lv_obj_t *cont_top = lv_obj_create(photomenu_flash_s);
    if(!cont_top) return;
    ...
    if (!btn_back)
        return;
    lv_obj_add_event_cb(btn_back, flash_btn_back_event_handler, LV_EVENT_CLICKED, ui);
    ...
    lv_label_set_text(title, str_language_flash[get_curr_language()]);
    ...
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(photomenu_flash_s);
    ...
    // 创建笑脸侦测按钮
    const char *btn_labels[] = {str_language_off[get_curr_language()], str_language_on[get_curr_language()], str_language_auto[get_curr_language()]};
    static lv_point_precise_t line_points_pool[sizeof(btn_labels) / sizeof(btn_labels[0])][3];
    for(int i = 0; i < 3; i++) {
        ...
        // 添加事件处理器
        lv_obj_add_event_cb(btn, flash_btn_event_handler, LV_EVENT_ALL, settings_cont);

        if(i == flash_Current_Index_s / 2) {
            {
    ...
    //先设置焦点控件,再进行滚动,否则会直接滚动到最下,不知什么原因.
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(settings_cont, flash_Current_Index_s);
    lv_group_focus_obj(chlid);
    lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
    lv_obj_scroll_to_y(settings_cont, ((flash_Current_Index_s / 2) * MENU_BTN_SIZE), LV_ANIM_OFF);
    // //设置焦点渐变
    // lv_set_obj_grad_style(chlid, LV_GRAD_DIR_VER, lv_color_hex(0xFBDEBD), lv_color_hex(0xF09F20));
    // //设置焦点BG
    // lv_obj_set_style_bg_color(chlid, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(photomenu_flash_s);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, flash_Current_Index_s);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS, flash_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(flash_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(photomenu_flash_s);
}
