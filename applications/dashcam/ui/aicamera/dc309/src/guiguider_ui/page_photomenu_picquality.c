...
static uint8_t picQual_Current_Index_s = 0;

void photo_setQuality_Index(uint8_t index)
{
    picQual_Current_Index_s = index * 2;
}

uint8_t photo_getQuality_Index(void)
{
    return picQual_Current_Index_s / 2;
}

void photo_setQuality_Label(const char* plabel)
{
    memset(g_button_labelPicQual, 0, sizeof(g_button_labelPicQual));
    strncpy(g_button_labelPicQual, plabel, sizeof(g_button_labelPicQual));
}

char* photo_getQuality_Label(void)
{
    return g_button_labelPicQual;
}

static void picQual_Del_Complete_anim_cb(lv_anim_t *a)
{
    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
                            &g_ui.screen_SettingPictureQuality_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
                            false, true);
}

static void picQual_win_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, picQual_Del_Complete_anim_cb);
...
static void screen_SettingPictureQuality_btn_back_event_handler(lv_event_t *e)
{
    ...
    switch(code) {
        case LV_EVENT_CLICKED: {
            ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
                                  &g_ui.screen_SettingPictureQuality_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0,
                                  0, false, true);
            break;
        }
        default: break;
...
{
    lv_obj_add_event_cb(ui->page_picturequality.btn_back, screen_SettingPictureQuality_btn_back_event_handler,
                        LV_EVENT_CLICKED, ui);
}

void photoPicqual_SelectFocus_OK(lv_event_t *e, lv_obj_t *obj)
{
    ...
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(parent, picQual_Current_Index_s);

    for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
        if(i == picQual_Current_Index_s) {
...
static void screen_SettingPictureQuality_btn_event_handler(lv_event_t *e)
{
        ...
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                ...
                if(lv_obj_get_child(parent, i) == btn_clicked) {
                    if (picQual_Current_Index_s == i) {
                        // 如果点击的是当前选中的按钮，则不做任何操作
                        break;
                    }
                    picQual_Current_Index_s = i;
                    photoPicqual_SelectFocus_OK(e, NULL);
                    // 获取按钮标签文本
                    lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
                    if(label && lv_obj_check_type(label, &lv_label_class)) {
                        MESSAGE_S Msg = {0};
                        const char *txt = lv_label_get_text(label);
                        if(txt) strncpy(g_button_labelPicQual, txt, sizeof(g_button_labelPicQual));

                        MLOG_DBG("event: %s\n", txt);

                        Msg.topic = EVENT_MODEMNG_SETTING;
                        Msg.arg1  = PARAM_MENU_PHOTO_QUALITY;
                        Msg.arg2  = picQual_Current_Index_s / 2;
...
static void photoPicqual_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("photoPicqual_click_callback\n");
    ...
    for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
        ...
        if(lv_obj_get_child(parent, i) == obj) {
            ...
            picQual_Current_Index_s = i;
            photoPicqual_SelectFocus_OK(NULL, obj);
            // 获取按钮标签文本
            lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
            if(label && lv_obj_check_type(label, &lv_label_class)) {
                MESSAGE_S Msg = {0};
...
static void photoPicqual_menu_callback(void)
{
    MLOG_DBG("photoPicqual_menu_callback\n");
    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
        &g_ui.screen_SettingPictureQuality_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
        false, true);
}
    ...
    switch(code) {
        case LV_EVENT_GESTURE: {
            ...
            switch(dir) {
                case LV_DIR_RIGHT: {
                    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
                                          &g_ui.screen_SettingPictureQuality_del, photoMenu_Setting,
...
{
    MLOG_DBG("loading page_SettingPictureQuality...\n");

    SettingPictureQuality_t *SettingPictureQuality = &ui->page_picturequality;
    SettingPictureQuality->qual_del                = true;

    // 创建主页面1 容器
    if(SettingPictureQuality->qual_scr != NULL) {
        if(lv_obj_is_valid(SettingPictureQuality->qual_scr)) {
            MLOG_DBG("page_SettingPictureQuality->scr 仍然有效，删除旧对象\n");
            lv_obj_del(SettingPictureQuality->qual_scr);
        } else {
            MLOG_DBG("page_SettingPictureQuality->scr 已被自动销毁，仅重置指针\n");
        }
        SettingPictureQuality->qual_scr = NULL;
    }

    // Write codes scr
    SettingPictureQuality->qual_scr = lv_obj_create(NULL);
    lv_obj_set_size( SettingPictureQuality->qual_scr , H_RES, V_RES);
    lv_obj_add_style( SettingPictureQuality->qual_scr , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_event_cb(SettingPictureQuality->qual_scr, gesture_event_handler, LV_EVENT_GESTURE, ui);

    // Write codes cont_top (顶部栏)
    SettingPictureQuality->cont_top = lv_obj_create(SettingPictureQuality->qual_scr);
    lv_obj_set_pos(SettingPictureQuality->cont_top, 0, 0);
    lv_obj_set_size(SettingPictureQuality->cont_top, H_RES, 60);
    lv_obj_set_scrollbar_mode(SettingPictureQuality->cont_top, LV_SCROLLBAR_MODE_OFF);
    lv_obj_add_style(SettingPictureQuality->cont_top, &style_common_cont_top, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes btn_back (返回按钮)
    SettingPictureQuality->btn_back = lv_button_create(SettingPictureQuality->cont_top);
    lv_obj_set_pos(SettingPictureQuality->btn_back, 4, 4);
    lv_obj_set_size(SettingPictureQuality->btn_back, 60, 52);
    lv_obj_add_style(SettingPictureQuality->btn_back, &style_common_btn_back, LV_PART_MAIN | LV_STATE_DEFAULT);

    SettingPictureQuality->label_back = lv_label_create(SettingPictureQuality->btn_back);
    lv_label_set_text(SettingPictureQuality->label_back, "" LV_SYMBOL_LEFT "");
    lv_label_set_long_mode(SettingPictureQuality->label_back, LV_LABEL_LONG_WRAP);
    lv_obj_align(SettingPictureQuality->label_back, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_width(SettingPictureQuality->label_back, LV_PCT(100));
    lv_obj_add_style(SettingPictureQuality->label_back, &style_common_label_back, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes title (标题)
    SettingPictureQuality->title = lv_label_create(SettingPictureQuality->cont_top);
    lv_label_set_text(SettingPictureQuality->title, str_language_quality[get_curr_language()]);
    lv_label_set_long_mode(SettingPictureQuality->title, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_font(SettingPictureQuality->title, get_usr_fonts(ALI_PUHUITI_FONTPATH, MENU_FONT_SIZE),
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(SettingPictureQuality->title, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(SettingPictureQuality->title, LV_ALIGN_CENTER, 0, 0);
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(SettingPictureQuality->qual_scr);
    ...
    // 创建画质选项按钮
    const char *btn_labels[] = {str_language_ultra_high_quality[get_curr_language()], str_language_high_quality[get_curr_language()], str_language_normal[get_curr_language()]};
    ...
    for(int i = 0; i < 3; i++) {
        ...
        lv_obj_add_event_cb(btn, screen_SettingPictureQuality_btn_event_handler, LV_EVENT_ALL, settings_cont);

        if(i == picQual_Current_Index_s / 2) {
            {
    ...
    //先设置焦点控件,再进行滚动,否则会直接滚动到最下,不知什么原因.
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(settings_cont, picQual_Current_Index_s);
    lv_group_focus_obj(chlid);
    lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
    lv_obj_scroll_to_y(settings_cont, ((picQual_Current_Index_s / 2) * MENU_BTN_SIZE), LV_ANIM_OFF);
    // //设置焦点渐变
    // lv_set_obj_grad_style(chlid, LV_GRAD_DIR_VER, lv_color_hex(0xFBDEBD), lv_color_hex(0xF09F20));
    // //设置焦点BG
    // lv_obj_set_style_bg_color(chlid, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(SettingPictureQuality->qual_scr);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, picQual_Current_Index_s);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS,
                     photoPicqual_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(photoPicqual_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(SettingPictureQuality->qual_scr);

    // Init events for screen.
    events_init_screen_SettingPictureQuality(ui);
}
