 ...
 */
 ...
 extern char g_sysbtn_labelcursor[32];
 
 lv_obj_t *obj_sysMenu_cursor_s;
 
 static uint8_t cursor_Current_Index_s = 0;
 
 uint8_t get_curr_cursor(void)
 {
     return cursor_Current_Index_s/2;
 }
 
 void cursor_Index(uint8_t index)
 {
     cursor_Current_Index_s = index * 2;
 }
 
 void cursor_Label(char* plabel)
 {
     memset(g_sysbtn_labelcursor, 0, sizeof(g_sysbtn_labelcursor));
...
static void cursor_Del_Complete_anim_cb(lv_anim_t* a)
{
 ...
 static void cursor_win_Delete_anim(void)
 {
     ...
     // 设置动画完成回调（销毁对象）
     lv_anim_set_completed_cb(&Delete_anim, cursor_Del_Complete_anim_cb);
...
static void sysMenu_cursor_btn_back_event_handler(lv_event_t *e)
{
 ...
 void syamenu_cursor_SelectFocus_OK(lv_event_t *e)
 {
     ...
     //获取焦点控件
     lv_obj_t *chlid = lv_obj_get_child(parent, cursor_Current_Index_s);
 
     for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
         if(i == cursor_Current_Index_s) {
 ...
 static void sysMenu_cursor_Select_btn_event_handler(lv_event_t *e)
 {
     lv_event_code_t code = lv_event_get_code(e);
     switch(code) {
         case LV_EVENT_CLICKED: {
 ...
 static void sysmenu_cursor_click_callback(lv_obj_t *obj)
 {
     MLOG_DBG("sysmenu_cursor_click_callback\n");
...
static void gesture_event_handler(lv_event_t *e)
{
 ...
 void photoMenu_Cursor(lv_ui_t *ui)
 {
    ...
    // Write codes resscr
     obj_sysMenu_cursor_s = lv_obj_create(NULL);
     lv_obj_set_size( obj_sysMenu_cursor_s , H_RES, V_RES);
     lv_obj_add_style( obj_sysMenu_cursor_s , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
     lv_obj_add_event_cb(obj_sysMenu_cursor_s, gesture_event_handler, LV_EVENT_GESTURE, ui);
 
     // Write codes cont_top (顶部栏)
     lv_obj_t *cont_top = lv_obj_create(obj_sysMenu_cursor_s);
     ...
     lv_obj_add_event_cb(btn_back, sysMenu_cursor_btn_back_event_handler, LV_EVENT_CLICKED, NULL);
     ...
     // 创建设置选项容器
     lv_obj_t *settings_cont = lv_obj_create(obj_sysMenu_cursor_s);
     ...
     const char* btn_labels[] = {
         str_language_off[get_curr_language()],
         "cur 1",
         "cur 2",
         "cur 3",
         "cur 4",
     ...
     lv_obj_t *chlid = lv_obj_get_child(settings_cont, cursor_Current_Index_s);
     lv_group_focus_obj(chlid);
     lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
     lv_obj_scroll_to_y(settings_cont, ((cursor_Current_Index_s / 2) * MENU_BTN_SIZE), LV_ANIM_OFF);
     lv_obj_set_style_text_color(lv_obj_get_child(chlid, 0), lv_color_hex(0xF09F20), LV_PART_MAIN | LV_STATE_DEFAULT);
 
     // 在上方添加一条分割线
     lv_obj_t *up_line                       = lv_line_create(obj_sysMenu_cursor_s);
     ...
     lv_obj_t *target_obj = lv_obj_get_child(settings_cont, cursor_Current_Index_s);
     // 初始化焦点组
     init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS, sysmenu_cursor_click_callback, target_obj);
     // 设置当前页面的按键处理器
     set_current_page_handler(handle_grid_navigation);
     takephoto_register_menu_callback(sysmenu_cursor_menu_callback);
 
     // Update current screen layout.
     lv_obj_update_layout(obj_sysMenu_cursor_s);
 }
