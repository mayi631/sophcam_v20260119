...
extern char g_sysbtn_labelLightfreq[32];

lv_obj_t *obj_sysMenu_LightFreq_s;

static uint8_t lightFreq_Current_Index_s = 2;


uint8_t getLightFreq_Index(void)
{
    return lightFreq_Current_Index_s / 2;
}

void SetLightFreq_Index(int32_t index)
{
    lightFreq_Current_Index_s = index * 2;
}

void SetLightFreq_Label(const char* plabel)
{
    memset(g_sysbtn_labelLightfreq, 0, sizeof(g_sysbtn_labelLightfreq));
    strncpy(g_sysbtn_labelLightfreq, plabel, sizeof(g_sysbtn_labelLightfreq));
}

char* GetLightFreq_Label(void)
{
    return g_sysbtn_labelLightfreq;
}

static void lightFreq_Del_Complete_anim_cb(lv_anim_t *a)
{
    ui_load_scr_animation(&g_ui, &obj_sysMenu_Setting_s, 1, NULL, sysMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
                            false, true);
}

static void lightFreq_win_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, lightFreq_Del_Complete_anim_cb);

    lv_anim_start(&Delete_anim);
}

static void sysMenu_LightFreq_btn_back_event_handler(lv_event_t *e)
{
...
static void sysMenu_LightFreq_Select_btn_event_handler(lv_event_t *e)
{
        ...
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                ...
                if(lv_obj_get_child(parent, i) == btn_clicked) {
                    if (lightFreq_Current_Index_s == i) {
                        // 如果点击的是当前选中的按钮，则不做任何操作
                        break;
                    }
                    lightFreq_Current_Index_s = i;
                    // 获取按钮标签文本
                    lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
                    if(label && lv_obj_check_type(label, &lv_label_class)) {
                        const char *txt = lv_label_get_text(label);
                        if(txt) SetLightFreq_Label(txt);

                        MLOG_DBG("event: %s\n", txt);

                        MESSAGE_S Msg = {0};
                        Msg.topic = EVENT_MODEMNG_SETTING;
                        Msg.arg1  = PARAM_MENU_LIGHT_FREQUENCE;
                        Msg.arg2  =  lightFreq_Current_Index_s / 2; // 0:50HZ, 1:60HZ
...
static void sysmenu_lightfreq_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("sysmenu_lightfreq_click_callback\n");
...
static void sysmenu_lightfreq_menu_callback(void)
{
    MLOG_DBG("sysmenu_lightfreq_menu_callback\n");
    ui_load_scr_animation(&g_ui, &obj_sysMenu_Setting_s, 1, NULL, sysMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
        false, true);
}
...
void sysMenu_LightFreq(lv_ui_t *ui)
{
    ...
    // 创建主页面1 容器
    if(obj_sysMenu_LightFreq_s != NULL) {
        if(lv_obj_is_valid(obj_sysMenu_LightFreq_s)) {
            MLOG_DBG("obj_sysMenu_LightFreq_s 仍然有效，删除旧对象\n");
            lv_obj_del(obj_sysMenu_LightFreq_s);
        } else {
            MLOG_DBG("obj_sysMenu_LightFreq_s 已被自动销毁，仅重置指针\n");
        }
        obj_sysMenu_LightFreq_s = NULL;
    }

    // Write codes resscr
    obj_sysMenu_LightFreq_s = lv_obj_create(NULL);
    lv_obj_set_size( obj_sysMenu_LightFreq_s , H_RES, V_RES);
    lv_obj_add_style( obj_sysMenu_LightFreq_s , &style_common_main_bg, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_add_event_cb(obj_sysMenu_LightFreq_s, gesture_event_handler, LV_EVENT_GESTURE, ui);
    
    // Write codes cont_top (顶部栏)
    lv_obj_t *cont_top = lv_obj_create(obj_sysMenu_LightFreq_s);
    ...
    lv_obj_add_event_cb(btn_back, sysMenu_LightFreq_btn_back_event_handler, LV_EVENT_CLICKED, NULL);

    // Write codes title (标题)
    lv_obj_t* title = lv_label_create(cont_top);
    lv_label_set_text(title, str_language_light_frequency[get_curr_language()]);
    ...
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(obj_sysMenu_LightFreq_s);
    ...
    // 创建分辨率选项按钮
    const char *btn_labels[] = {"50HZ", "60HZ"};
    ...
    for(uint8_t i = 0; i < sizeof(btn_labels) / sizeof(btn_labels[0]); i++) {
        ...
        lv_obj_add_event_cb(btn, sysMenu_LightFreq_Select_btn_event_handler, LV_EVENT_ALL, settings_cont);

        if(i == lightFreq_Current_Index_s / 2) {
            {
    ...
    //先设置焦点控件,再进行滚动,否则会直接滚动到最下,不知什么原因.
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(settings_cont, lightFreq_Current_Index_s);
    lv_group_focus_obj(chlid);
    lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
    lv_obj_scroll_to_y(settings_cont, (lightFreq_Current_Index_s * MENU_BTN_SIZE), LV_ANIM_OFF);
    // //设置焦点渐变
    // lv_set_obj_grad_style(chlid, LV_GRAD_DIR_VER, lv_color_hex(0xFBDEBD), lv_color_hex(0xF09F20));
    // //设置焦点BG
    // lv_obj_set_style_bg_color(chlid, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(obj_sysMenu_LightFreq_s);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, lightFreq_Current_Index_s);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS, sysmenu_lightfreq_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(sysmenu_lightfreq_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(obj_sysMenu_LightFreq_s);
