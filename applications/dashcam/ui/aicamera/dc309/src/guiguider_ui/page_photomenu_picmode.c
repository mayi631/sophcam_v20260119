...
static uint8_t picMode_Current_Index_s = 0;
...
static void picMode_win_Delete_anim(void)
{
    ...
    // 设置动画完成回调（销毁对象）
    lv_anim_set_completed_cb(&Delete_anim, picMode_Del_Complete_anim_cb);
...
{
    lv_obj_add_event_cb(ui->page_picturemode.btn_back, screen_SettingPictureMode_btn_back_event_handler,
                        LV_EVENT_CLICKED, ui);
}

void photoPicMode_SelectFocus_OK(lv_event_t *e, lv_obj_t *obj)
{
    ...
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(parent, picMode_Current_Index_s);

    for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
        if(i == picMode_Current_Index_s) {
...
// 添加按钮事件处理函数
static void screen_SettingPictureMode_btn_event_handler(lv_event_t *e)
{
        ...
        case LV_EVENT_CLICKED: {
            ...
            for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {
                ...
                if(lv_obj_get_child(parent, i) == btn_clicked) {
                    picMode_Current_Index_s = i;
                    photoPicMode_SelectFocus_OK(e, NULL);
                    // 获取按钮标签文本
                    lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
                    if(label && lv_obj_check_type(label, &lv_label_class)) {
                        const char *txt = lv_label_get_text(label);
                        if(txt) strncpy(g_button_labelPicMode, txt, sizeof(g_button_labelPicMode));
                    }
...
static void photoPicMode_click_callback(lv_obj_t *obj)
{
    MLOG_DBG("photoPicMode_click_callback\n");
    lv_obj_t *parent      = lv_obj_get_parent(obj); //获取发生点击事件的父控件
    for(uint8_t i = 0; i < lv_obj_get_child_cnt(parent); i++) {

        if(lv_obj_get_child(parent, i) == obj) {
            picMode_Current_Index_s = i;
            photoPicMode_SelectFocus_OK(NULL, obj);
            // 获取按钮标签文本
            lv_obj_t *label = lv_obj_get_child(lv_obj_get_child(parent, i), 0);
            if(label && lv_obj_check_type(label, &lv_label_class)) {
                const char *txt = lv_label_get_text(label);
...
static void photoPicMode_menu_callback(void)
{
    MLOG_DBG("photoPicMode_menu_callback\n");
    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
        &g_ui.screen_SettingPictureMode_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE, 0, 0,
        false, true);
}
    ...
    switch(code) {
        case LV_EVENT_GESTURE: {
            ...
            switch(dir) {
                case LV_DIR_RIGHT: {
                    ui_load_scr_animation(&g_ui, &g_ui.page_photoMenu_Setting.menuscr, g_ui.screenPhotoMenuSetting_del,
                                          &g_ui.screen_SettingPictureMode_del, photoMenu_Setting, LV_SCR_LOAD_ANIM_NONE,
                                          0, 0, false, true);
                }
                default: break;
            }
...
void photoMenu_PictureMode(lv_ui_t *ui)
{
    MLOG_DBG("loading page_picturemode...\n");

    SettingPictureMode_t *SettingPictureMode = &ui->page_picturemode;
    SettingPictureMode->picmod_del           = true;

    // 创建主页面1 容器
    if(SettingPictureMode->picmod_scr != NULL) {
        if(lv_obj_is_valid(SettingPictureMode->picmod_scr)) {
            MLOG_DBG("page_picturemode->scr 仍然有效，删除旧对象\n");
            lv_obj_del(SettingPictureMode->picmod_scr);
        } else {
            MLOG_DBG("page_picturemode->scr 已被自动销毁，仅重置指针\n");
        }
        SettingPictureMode->picmod_scr = NULL;
    }

    // Write codes scr
    SettingPictureMode->picmod_scr = lv_obj_create(NULL);
    lv_obj_set_size(SettingPictureMode->picmod_scr, 640, 480);
    lv_obj_set_scrollbar_mode(SettingPictureMode->picmod_scr, LV_SCROLLBAR_MODE_OFF);
    lv_obj_set_style_pad_all(SettingPictureMode->picmod_scr, 0, 0);
    lv_obj_add_event_cb(SettingPictureMode->picmod_scr, gesture_event_handler, LV_EVENT_GESTURE, ui);

    // Write style for scr, Part: LV_PART_MAIN, State: LV_STATE_DEFAULT.
    lv_obj_set_style_bg_opa(SettingPictureMode->picmod_scr, LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(lv_layer_bottom(), LV_OPA_TRANSP, LV_PART_MAIN);
    lv_obj_set_style_bg_opa(SettingPictureMode->picmod_scr, LV_OPA_100, LV_PART_MAIN);
    lv_obj_set_style_bg_color(SettingPictureMode->picmod_scr, lv_color_hex(0x000000), LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes cont_top (顶部栏)
    SettingPictureMode->cont_top = lv_obj_create(SettingPictureMode->picmod_scr);
    lv_obj_set_pos(SettingPictureMode->cont_top, 0, 0);
    lv_obj_set_size(SettingPictureMode->cont_top, 640, 60);
    lv_obj_set_scrollbar_mode(SettingPictureMode->cont_top, LV_SCROLLBAR_MODE_OFF);
    lv_obj_add_style(SettingPictureMode->cont_top, &style_common_cont_top, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes btn_back (返回按钮)
    SettingPictureMode->btn_back = lv_button_create(SettingPictureMode->cont_top);
    lv_obj_set_pos(SettingPictureMode->btn_back, 4, 4);
    lv_obj_set_size(SettingPictureMode->btn_back, 60, 52);
    lv_obj_add_style(SettingPictureMode->btn_back, &style_common_btn_back, LV_PART_MAIN | LV_STATE_DEFAULT);

    SettingPictureMode->label_back = lv_label_create(SettingPictureMode->btn_back);
    lv_label_set_text(SettingPictureMode->label_back, "" LV_SYMBOL_LEFT "");
    lv_label_set_long_mode(SettingPictureMode->label_back, LV_LABEL_LONG_WRAP);
    lv_obj_align(SettingPictureMode->label_back, LV_ALIGN_CENTER, 0, 0);
    lv_obj_set_width(SettingPictureMode->label_back, LV_PCT(100));
    lv_obj_add_style(SettingPictureMode->label_back, &style_common_label_back, LV_PART_MAIN | LV_STATE_DEFAULT);

    // Write codes title (标题)
    SettingPictureMode->title = lv_label_create(SettingPictureMode->cont_top);
    lv_label_set_text(SettingPictureMode->title, str_language_picture_mode[get_curr_language()]);
    lv_label_set_long_mode(SettingPictureMode->title, LV_LABEL_LONG_WRAP);
    lv_obj_set_style_text_font(SettingPictureMode->title, get_usr_fonts(ALI_PUHUITI_FONTPATH, MENU_FONT_SIZE),
                               LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(SettingPictureMode->title, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(SettingPictureMode->title, LV_ALIGN_CENTER, 0, 0);
    // 创建设置选项容器
    lv_obj_t *settings_cont = lv_obj_create(SettingPictureMode->picmod_scr);
    ...
    // 创建场景模式按钮
    const char *btn_labels[] = {str_language_auto[get_curr_language()], str_language_beach[get_curr_language()], str_language_night_scene[get_curr_language()], str_language_landscape[get_curr_language()], str_language_portrait[get_curr_language()], str_language_sports[get_curr_language()], str_language_backlight[get_curr_language()], str_language_party[get_curr_language()]};
    ...
    for(int i = 0; i < 8; i++) {
        ...
        lv_obj_add_event_cb(btn, screen_SettingPictureMode_btn_event_handler, LV_EVENT_ALL, settings_cont);

        if(i == picMode_Current_Index_s / 2) {
            {
    ...
    //先设置焦点控件,再进行滚动,否则会直接滚动到最下,不知什么原因.
    //获取焦点控件
    lv_obj_t *chlid = lv_obj_get_child(settings_cont, picMode_Current_Index_s);
    lv_group_focus_obj(chlid);
    lv_obj_add_state(chlid, LV_STATE_FOCUS_KEY);
    lv_obj_scroll_to_y(settings_cont, ((picMode_Current_Index_s / 2) * MENU_BTN_SIZE), LV_ANIM_OFF);
    // //设置焦点渐变
    // lv_set_obj_grad_style(chlid, LV_GRAD_DIR_VER, lv_color_hex(0xFBDEBD), lv_color_hex(0xF09F20));
    // //设置焦点BG
    // lv_obj_set_style_bg_color(chlid, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    ...
    // 在上方添加一条分割线
    lv_obj_t *up_line                       = lv_line_create(SettingPictureMode->picmod_scr);
    ...
    lv_obj_t *target_obj = lv_obj_get_child(settings_cont, picMode_Current_Index_s);
    // 初始化焦点组
    init_focus_group(settings_cont, GRID_COLS, GRID_ROWS, focusable_objects, GRID_MAX_OBJECTS,
                     photoPicMode_click_callback, target_obj);
    // 设置当前页面的按键处理器
    set_current_page_handler(handle_grid_navigation);
    takephoto_register_menu_callback(photoPicMode_menu_callback);

    // Update current screen layout.
    lv_obj_update_layout(SettingPictureMode->picmod_scr);

    // Init events for screen.
    events_init_screen_SettingPictureMode(ui);
}
